...
tar -cpzf $photos_dest/$archive_photos $backup_photos
...
OLD_FILE=$(find "$photos_dest" -type f -print0 | xargs -0 ls -lt | tail -n 1 | awk '{print $9}')
if [ -z "$OLD_FILE" ]; then
   echo "could not find old backup in dest: $photos_dest"
fi
rm "$OLD_FILE"
